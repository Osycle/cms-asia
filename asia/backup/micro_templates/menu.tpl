...
<div class="min-menu hidden-md hidden-lg">
	<div class="bg-black clearfix"></div>
  	<div class="min-menu-logo"><a href="{$SITE_URL}"><img src="{$SITE_URL}/images/logo.png" alt="{$config.website_name}"/></a></div>
	...
	<section class="drawer-list">
	 	<hr class="line-shadow-1">
	  	<div class="min-menu-skin">
			<ul>
				<li {if $webpage}class="active"{/if}>{foreach item=webpage from=$allWebpages name=webpages}{if $webpage.top_menu eq '1'}<a href="{$SITE_URL}">{$webpage.alias}</a>{/if}{/foreach}</li>
				{foreach item=navItem1 from=$sectionTree name=navItems1}{if $navItem1.top_menu eq '1'}
				<li {if $section.fileName eq $navItem1.fileName} class="active"{/if}><a href="{$navItem1.url}">{$navItem1.alias}</a></li>
				{/if}{/foreach}
			</ul>
